...
RSpec.describe 'QuizScraper::PubQuizzer' do
  ...
  context "on +find_all+" do
    let(:find_all_response) {
      <<-BORK
        <html>
          <head></head>
          <body>
    ...
    before(:each) do
      stub_request(:get, "http://www.pubquizzers.com/search.php").with(
        headers: {
          'Accept'=>'*/*',
          'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
          'User-Agent'=>'Ruby'
        }
      ).to_return(status: 200, :body => find_all_response, headers: {})
    end
  ...
  context "on +find+" do
    ...
    before(:each) do
      stub_request(:get, "http://www.pubquizzers.com/pub-quiz/84/the-pilot-inn").with(
        headers: {
          'Accept'=>'*/*',
          'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3',
          'User-Agent'=>'Ruby'
        }
      ).to_return(status: 200, :body => find_response, headers: {})
    end
